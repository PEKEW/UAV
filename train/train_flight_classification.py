...
from models.model import FlightAnomalyLSTM
from data.dataset import get_flight_dataset

def train_model(model, train_loader, val_loader, num_epochs=40, lr=0.001, 
                device='cuda', save_path='flight_anomaly_model.pth'):
    """训练飞行姿态异常检测模型"""
...
def plot_results(training_history, evaluation_results, title_prefix="Flight Attitude"):
    """绘制训练和评估结果"""
    ...
    axes[0, 0].set_title(f'{title_prefix} Training and Validation Loss')
    ...
    axes[0, 1].set_title(f'{title_prefix} Training and Validation Accuracy')
    ...
    axes[0, 2].set_title(f'{title_prefix} Confusion Matrix')
    ...
    axes[1, 0].set_title(f'{title_prefix} ROC Curve')
    ...
    axes[1, 1].set_title(f'{title_prefix} Confidence Distribution')
    ...
    if evaluation_results['uncertainty'] is not None:
        ...
        axes[1, 2].set_title(f'{title_prefix} Uncertainty Distribution')
    ...
    plt.tight_layout()
    plt.savefig('flight_anomaly_results.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    # 数据路径
    data_path = "."  # 当前目录包含flight_attitude_anomaly_dataset.csv
    
    # 获取数据集
    print("加载飞行姿态数据集...")
    train_dataset, val_dataset, test_dataset = get_flight_dataset(
        data_path, 
        sequence_length=50,
        train_ratio=0.7,
        val_ratio=0.15,
    ...
    # 训练模型
    print("开始训练飞行姿态异常检测模型...")
    training_history = train_model(
        model, train_loader, val_loader,
        num_epochs=40,
        lr=0.001,
        device=device,
        save_path='flight_anomaly_model.pth'
    )
    
    # 加载最佳模型
    print("加载最佳模型进行评估...")
    checkpoint = torch.load('flight_anomaly_model.pth')
    ...
    # 绘制结果
    print("生成结果图表...")
    plot_results(training_history, evaluation_results, "Flight Attitude")
    ...
    results_df.to_csv('flight_anomaly_results.csv', index=False)
    print("结果已保存为: flight_anomaly_results.csv")
